 ...
 * @example
 * ```typescript
 * import { unicode } from 'blecsd/utils';
 *
 * // Width operations
 * const width = unicode.width.stringWidth('Hello 世界');
 * const truncated = unicode.width.truncateByWidth(text, 20);
 *
 * // Code point operations
 * const codePoints = unicode.codePoint.toCodePoints('Hello');
 * const char = unicode.codePoint.fromCodePoint(0x1f600);
 *
 * // Grapheme operations
 * const clusters = unicode.grapheme.graphemeClusters('Hello 👨‍👩‍👧‍👦');
 * const count = unicode.grapheme.graphemeCount(text);
 *
 * // Normalization operations
 * const normalized = unicode.normalize.normalizeNFC(text);
 *
 * // Surrogate pair operations
 * const pair = unicode.surrogate.codePointToSurrogatePair(0x1f600);
 * const codePoint = unicode.surrogate.surrogatePairToCodePoint(0xd83d, 0xde00);
 * ```
 */
...
export type UnicodeModule = typeof unicode;
