...
describe('VisibilityCulling', () => {
	describe('PositionCache', () => {
		...
		const grid = createSpatialHash({ cellSize: 8 });
			...
			const updated1 = updateEntityIfMoved(world, grid, cache, eid, 10, 20, 4, 4);
			expect(updated1).toBe(true);

			// Same position - no update
			const updated2 = updateEntityIfMoved(world, grid, cache, eid, 10, 20, 4, 4);
			expect(updated2).toBe(false);

			// Moved - updates
			const updated3 = updateEntityIfMoved(world, grid, cache, eid, 15, 20, 4, 4);
		...
		const grid = createSpatialHash({ cellSize: 8 });
			...
			updateEntityIfMoved(world, grid, cache, eid, 10, 20, 4, 4);
			removeFromCache(cache, eid);

			// After removal, inserting same position counts as new
			const updated = updateEntityIfMoved(world, grid, cache, eid, 10, 20, 4, 4);
		...
		const grid = createSpatialHash({ cellSize: 8 });
			...
			updateEntityIfMoved(world, grid, cache, 1 as Entity, 10, 20, 4, 4);
			updateEntityIfMoved(world, grid, cache, 2 as Entity, 30, 40, 4, 4);
	...
	describe('queryVisibleEntities', () => {
		...
		const grid = createSpatialHash({ cellSize: 8 });
			...
			insertEntity(world, grid, 1 as Entity, 10, 10, 4, 4);
			insertEntity(world, grid, 2 as Entity, 50, 50, 4, 4);
			insertEntity(world, grid, 3 as Entity, 200, 200, 4, 4);
		...
		const grid = createSpatialHash({ cellSize: 8 });
			...
			insertEntity(world, grid, 1 as Entity, 100, 100, 4, 4);
	...
	describe('performCulling', () => {
		...
		const grid = createSpatialHash({ cellSize: 8 });
			...
			insertEntity(world, grid, 1 as Entity, 5, 5, 2, 2);
			insertEntity(world, grid, 2 as Entity, 100, 100, 2, 2);
