...
describe('SmoothScroll', () => {
	...
	describe('getScrollState', () => {
		...
		it('returns existing state on second call', () => {
			const state1 = getScrollState(eid, 100, 500, 80, 24);
			state1.scrollY = 50;

			const state2 = getScrollState(eid, 100, 500, 80, 24);
	...
	describe('applyScrollImpulse', () => {
		...
		it('clamps to max velocity', () => {
			...
			applyScrollImpulse(eid, 0, -999, { maxVelocity: 100 });
	...
	describe('updateScrollPhysics', () => {
		...
		it('clamps scroll to content bounds', () => {
			const state = getScrollState(eid, 80, 100, 80, 24);
