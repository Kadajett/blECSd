...
describe('DoubleBuffer', () => {
	...
	describe('Dirty region tracking', () => {
		...
		it('markLineDirty marks entire line width', () => {
			const db = createDoubleBuffer(80, 24);
			clearDirtyRegions(db);
			...
			const regions = getDirtyRegions(db);
			expect(regions.length).toBe(1);
			expect(regions[0]).toEqual({ x: 0, y: 10, w: 80, h: 1 });
		});

		it('markLineDirty ignores out of bounds lines', () => {
			const db = createDoubleBuffer(80, 24);
			clearDirtyRegions(db);

			markLineDirty(db, -1);
			markLineDirty(db, 24);
			markLineDirty(db, 100);

			expect(getDirtyRegions(db).length).toBe(0);
		});

		it('getDirtyLines returns unique sorted line numbers', () => {
			const db = createDoubleBuffer(80, 24);
			clearDirtyRegions(db);

			markDirtyRegion(db, 0, 5, 10, 3); // Lines 5, 6, 7
			markLineDirty(db, 10); // Line 10
			markDirtyRegion(db, 20, 6, 5, 2); // Lines 6, 7 (overlaps)

			const lines = getDirtyLines(db);
			expect(lines).toEqual([5, 6, 7, 10]);
		});

		it('getDirtyLines returns empty for no dirty regions', () => {
			const db = createDoubleBuffer(80, 24);
			clearDirtyRegions(db);
		...
		it('getDirtyLines returns all lines for full redraw', () => {
			const db = createDoubleBuffer(80, 24);
			clearDirtyRegions(db);
			markFullRedraw(db);
