 ...
 * @example
 * ```typescript
 * import { throttledResizeNs } from 'blecsd/terminal';
 *
 * // Create throttled resize handler (max 10 resizes per second)
 * const throttled = throttledResizeNs.createThrottledResize((cols, rows) => {
 ...
 * const debounced = throttledResizeNs.debounceResize((cols, rows) => {
 ...
 * Throttled resize namespace.
 */
export const throttledResizeNs = Object.freeze({
	createThrottledResize,
	debounceResize,
	throttleResize,
});

export type ThrottledResizeNsModule = typeof throttledResizeNs;
