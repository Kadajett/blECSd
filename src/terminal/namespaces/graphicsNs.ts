 ...
 * @example
 * ```typescript
 * import { graphicsNs } from 'blecsd/terminal';
 *
 * // Create graphics manager with auto-detection
 * const manager = graphicsNs.createAutoGraphicsManager(program);
 *
 * // Render images
 * const imageData = { width: 100, height: 100, data: buffer };
 * graphicsNs.renderImage(manager, imageData, { x: 10, y: 5 });
 *
 * // Create Braille canvas for vector graphics
 * const canvas = graphicsNs.createBrailleCanvas(80, 40);
 * graphicsNs.drawBrailleLine(canvas, 0, 0, 79, 39);
 * graphicsNs.drawBrailleCircle(canvas, 40, 20, 10);
 * const cells = graphicsNs.brailleCanvasToCells(canvas);
 *
 * // Detect graphics support
 * const support = graphicsNs.detectGraphicsSupport();
 * if (graphicsNs.hasPixelBackend(manager)) {
 ...
 * Graphics protocol namespace.
 */
export const graphicsNs = Object.freeze({
	// Backend names
	ANSI_BACKEND_NAME,
	BRAILLE_BACKEND_NAME,
	ITERM2_BACKEND_NAME,
...
export type GraphicsNsModule = typeof graphicsNs;
