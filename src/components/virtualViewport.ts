...
export function setVirtualViewport(
	...
	if (options.totalLineCount !== undefined) {
		VirtualViewport.totalLineCount[eid] = options.totalLineCount;
	}
	if (options.visibleLineCount !== undefined) {
		VirtualViewport.visibleLineCount[eid] = options.visibleLineCount;
	}
	if (options.firstVisibleLine !== undefined) {
		VirtualViewport.firstVisibleLine[eid] = options.firstVisibleLine;
	}
	if (options.overscanBefore !== undefined) {
		VirtualViewport.overscanBefore[eid] = options.overscanBefore;
	}
	if (options.overscanAfter !== undefined) {
		VirtualViewport.overscanAfter[eid] = options.overscanAfter;
	}
	if (options.estimatedLineHeight !== undefined) {
		VirtualViewport.estimatedLineHeight[eid] = options.estimatedLineHeight;
	}
	if (options.isVariableHeight !== undefined) {
		VirtualViewport.isVariableHeight[eid] = options.isVariableHeight ? 1 : 0;
	}
	if (options.selectedLine !== undefined) {
		VirtualViewport.selectedLine[eid] = options.selectedLine;
	}
	if (options.cursorLine !== undefined) {
		VirtualViewport.cursorLine[eid] = options.cursorLine;
	}
