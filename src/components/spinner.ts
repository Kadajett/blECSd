...
import { addComponent, defineComponent, hasComponent, removeComponent, Types } from 'bitecs';
 ...
 */
export function addSpinner(world: World, eid: Entity, options: SpinnerOptions = {}): void {
	const frames = options.frames ?? DEFAULT_SPINNER_CHARS;
	const interval = options.interval ?? DEFAULT_SPINNER_INTERVAL;

	addComponent(world, Spinner, eid);
	Spinner.frame[eid] = 0;
 ...
 * @param eid - Entity ID
 */
export function removeSpinner(world: World, eid: Entity): void {
	if (hasComponent(world, Spinner, eid)) {
		removeComponent(world, Spinner, eid);
 ...
 */
export function hasSpinner(world: World, eid: Entity): boolean {
	return hasComponent(world, Spinner, eid);
}
