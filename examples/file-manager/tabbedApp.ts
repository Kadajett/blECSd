...
import type { Entity, World, KeyEvent, MouseEvent, CellBuffer } from 'blecsd';
import {
	createWorld,
	parseKeyBuffer,
	parseMouseSequence,
...
import type { ListItem } from 'blecsd';
...
function createRenderState(width: number, height: number, splitRatio: number): RenderState {
	...
	return {
		buffer: createCellBuffer(width, height),
		width,
		height,
		listWidth,
		previewWidth,
...
function updateRenderState(state: RenderState, width: number, height: number, splitRatio: number): void {
	if (state.width !== width || state.height !== height) {
		state.buffer = createCellBuffer(width, height);
		state.width = width;
		state.height = height;
	}
	state.listWidth = Math.max(20, Math.floor((width - 1) * splitRatio));
	state.previewWidth = Math.max(0, width - state.listWidth - 1);
...
function resetListForTab(world: World, tab: TabState, visibleCount: number): void {
	...
	setTotalCount(tab.listEid, tab.fileStore.count);
	setVisibleCount(tab.listEid, visibleCount);
...
function buildPreviewScrollback(preview: PreviewState, content: PreviewContent): void {
	...
	const grammar = detectLanguage(content.name || content.extension);
	setGrammar(preview.highlightCache, grammar);
}
...
async function createTab(
	...
	setTotalCount(listEid, fileStore.count);
	setLazyLoadCallback(world, listEid, async (start, count) => buildListItems(fileStore, start, count));
...
async function createAppState(initialPath: string, width: number, height: number): Promise<AppState> {
	...
	const renderState = createRenderState(width, height, config.splitRatio);
...
function handleMouseInput(state: AppState, event: MouseEvent): void {
	...
	if (event.action === 'wheel' && (event.button === 'wheelUp' || event.button === 'wheelDown')) {
		...
		if (state.config.showPreview && event.x > listEndX) {
			...
			scrollListBy(tab.listEid, delta);
		}
		state.needsRedraw = true;
		return;
	}
...
function scrollListBy(listEid: Entity, delta: number): void {
	const firstVisible = (getScrollInfo(listEid).firstVisible ?? 0) as number;
	const visibleCount = getScrollInfo(listEid).visibleCount ?? 0;
	const totalCount = getScrollInfo(listEid).totalCount ?? 0;
	const newFirst = Math.max(0, Math.min(totalCount - visibleCount, firstVisible + delta));
	setFirstVisible({} as World, listEid, newFirst);
}
...
function renderTabBar(state: AppState, tab: TabState, width: number): void {
	const y = 0;
...
function renderList(
	...
	for (let row = 0; row < height; row++) {
		...
		const fg = isCurrent ? COLORS.rowCurrentFg : COLORS.rowFg;
		...
		const nameX = x + 2;
		renderText(buffer, nameX - 2, y + row, icon, fileFg(entry), bg);
...
function fileFg(entry: { isExecutable: boolean; type: FileType }): number {
...
async function main(): Promise<void> {
	...
	const initialPath = process.argv[2] && !process.argv[2]?.startsWith('-') ? process.argv[2] : getHomePath();
	...
	stdout.on('resize', () => {
		...
		updateRenderState(state.renderState, width, height, state.config.splitRatio);
		state.actionBar.setPosition(0, height - 1);
		for (const tab of state.tabs) {
			setVisibleCount(tab.listEid, state.renderState.listHeight);
		}
		state.needsRedraw = true;
	});
