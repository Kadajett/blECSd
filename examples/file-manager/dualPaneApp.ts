...
import { getCurrentIndex, getVisibleRange, setVirtualList, ensureIndexVisible, setTotalItems, setSelection } from './components';
...
async function createAppState(
	...
	width: number,
	height: number,
	dualPaneConfig?: Partial<DualPaneConfig>,
): Promise<AppState> {
	const world = createWorld() as World;
	const config = { ...DEFAULT_CONFIG, showPreview: true };
	const dualPaneState = createDualPaneState(initialPath, dualPaneConfig);
	...
	// Create preview entity if preview is enabled
	if (dualPaneState.config.showPreview) {
		const previewEid = addEntity(world);
		setPreview(world, previewEid);
		dualPaneState.previewEntity = previewEid;
	}

	// Initialize all initial panes
	for (const pane of dualPaneState.panes) {
		const fileStore = createFileStore();
		await fileStore.loadDirectory(initialPath, config);
		dualPaneState.fileStores.set(pane.id, fileStore);

		const listEid = addEntity(world);
		dualPaneState.paneEntities.set(pane.id, listEid);

		// Calculate list height accounting for preview
		const layoutBounds = calculateLayoutBounds(dualPaneState, width, height, 0, 0, 2, 2);
		const paneBound = layoutBounds.panes.find((b) => b.paneId === pane.id);
		const listHeight = paneBound ? paneBound.height - 3 : height - 6;
		...
		virtualListStates.set(pane.id, vls);

		previewStates.set(pane.id, createPreviewState());
	}

	// Update preview for the active pane
	const activePane = getActivePane(dualPaneState);
	if (activePane && dualPaneState.previewEntity !== null) {
		const fileStore = dualPaneState.fileStores.get(activePane.id);
		const listEid = dualPaneState.paneEntities.get(activePane.id);
		const previewState = previewStates.get(activePane.id);

		if (fileStore && listEid !== undefined && previewState) {
			updatePreview(world, listEid, dualPaneState.previewEntity, fileStore, config.sizeFormat, previewState);
		}
	}
...
async function addNewPane(
	...
	for (const pane of newDualPaneState.panes) {
		if (!newDualPaneState.fileStores.has(pane.id)) {
			...
			// Calculate list height accounting for preview
			const layoutBounds = calculateLayoutBounds(newDualPaneState, screenWidth, screenHeight, 0, 0, 2, 2);
			const paneBound = layoutBounds.panes.find((b) => b.paneId === pane.id);
			const listHeight = paneBound ? paneBound.height - 3 : screenHeight - 6;

			setVirtualList(appState.world, listEid, { totalItems: fileStore.count, visibleCount: listHeight });

			// Initialize selection to first item
			setSelection(appState.world, listEid, 0);
...
/**
 * Updates preview for the active pane.
 */
function updateActivePreview(appState: AppState): void {
	const activePane = getActivePane(appState.dualPaneState);
	if (!activePane || appState.dualPaneState.previewEntity === null) return;
	...
	const previewState = appState.previewStates.get(activePane.id);

	if (fileStore && listEid !== undefined && previewState) {
		updatePreview(
			appState.world,
			listEid,
			appState.dualPaneState.previewEntity,
			fileStore,
			appState.config.sizeFormat,
			previewState,
		);
	}
}
...
async function handleKeyInput(
	...
	// Preview scrolling (Ctrl+j/k or Ctrl+Up/Down)
	if (event.ctrl && (key === 'j' || key === 'down') && appState.dualPaneState.previewEntity !== null) {
		const layoutBounds = calculateLayoutBounds(appState.dualPaneState, screenWidth, screenHeight, 0, 0, 2, 2);
		const visibleHeight = layoutBounds.preview ? layoutBounds.preview.height - 6 : 20;
		scrollPreviewDown(appState.world, appState.dualPaneState.previewEntity, 1, visibleHeight);
		appState.needsRedraw = true;
		return;
	}

	if (event.ctrl && (key === 'k' || key === 'up') && appState.dualPaneState.previewEntity !== null) {
		const layoutBounds = calculateLayoutBounds(appState.dualPaneState, screenWidth, screenHeight, 0, 0, 2, 2);
		const visibleHeight = layoutBounds.preview ? layoutBounds.preview.height - 6 : 20;
		scrollPreviewUp(appState.world, appState.dualPaneState.previewEntity, 1, visibleHeight);
		appState.needsRedraw = true;
		return;
	}

	// Tab - switch panes and update preview
	if (key === 'tab') {
		...
		// Update preview for newly active pane
		updateActivePreview(appState);
		appState.needsRedraw = true;
		return;
	}
	...
	if (key === '+' || (event.shift && key === '=')) {
		await addNewPane(appState, activePane.path, screenWidth, screenHeight);
		return;
	}
	...
	if (key === 'enter' || key === 'return' || key === 'l') {
		...
		if (result.success && !result.fileOpened) {
			// Directory changed
			const { state } = processDualPaneAction(
				appState.dualPaneState,
				appState.dragState,
	...
	if (key === '.') {
		...
		ensureIndexVisible(appState.world, listEid, getCurrentIndex(appState.world, listEid));
		// Update preview after hidden files toggle
		updateActivePreview(appState);
		appState.needsRedraw = true;
		return;
	}
...
function handleMouseInput(
	...
	// Determine which pane was clicked using layout bounds
	const layoutBounds = calculateLayoutBounds(appState.dualPaneState, screenWidth, screenHeight, 0, 0, 2, 2);
	...
	if (clickedPane && event.action === 'mousedown') {
		...
		const paneBounds = layoutBounds.panes.find((b) => b.paneId === clickedPane.id);
		if (paneBounds) {
			...
			if (localY >= 0 && localY < paneBounds.height - 3) {
				...
				if (listEid !== undefined && virtualListState && fileStore) {
					...
					ensureIndexVisible(appState.world, listEid, getCurrentIndex(appState.world, listEid));
					// Update preview when selection changes via mouse click
					updateActivePreview(appState);
					appState.needsRedraw = true;
				}
			}
 ...
 */
function parseArgs(): { initialPath: string; dualPaneConfig: Partial<DualPaneConfig> } {
	const args = process.argv.slice(2);
	let initialPath = getHomePath();
	const dualPaneConfig: Partial<DualPaneConfig> = {};

	for (let i = 0; i < args.length; i++) {
		const arg = args[i];
		if (arg === '--dual' || arg === '-d') {
			dualPaneConfig.initialPaneCount = 2;
		} else if (arg === '--no-preview') {
			dualPaneConfig.showPreview = false;
		} else if (arg?.startsWith('--panes=')) {
			const count = Number.parseInt(arg.slice(8), 10);
			if (!Number.isNaN(count) && count >= 1 && count <= 4) {
				dualPaneConfig.initialPaneCount = count;
			}
	...
	return { initialPath, dualPaneConfig };
}
...
function cleanupTerminal(): void {
	if (cleanupCalled || !terminalSetup) return;
	cleanupCalled = true;
...
async function main(): Promise<void> {
	...
	const { initialPath, dualPaneConfig } = parseArgs();

	// Initialize application
	const appState = await createAppState(initialPath, width, height, dualPaneConfig);
		...
		try {
			...
			if (str.startsWith('\x1b[<')) {
				...
				for (const keyEvent of keyEvents) {
					...
					void (async () => {
						try {
							await handleKeyInput(appState, keyEvent, width, height);
				...
				return; // Async key handling will redraw
			}
	...
	function cleanup(): void {
		...
		// Terminal cleanup handled by global handlers
		cleanupTerminal();
	}
}
...
// Export config type for use from index.ts
export type { DualPaneConfig } from './systems/dualPaneSystem';
export { DEFAULT_DUAL_PANE_CONFIG } from './systems/dualPaneSystem';

/**
 * Runs the dual-pane file manager application.
 */
export async function runDualPaneApp(): Promise<void> {
	return main();
}
