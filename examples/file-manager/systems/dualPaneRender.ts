...
export function renderDualPane(
	...
	const paneBounds = calculatePaneBounds(dualPaneState, width, height, 0, 0, 2, 2);
	...
	renderPaneDividers(buffer, paneBounds, height);

	// Render add pane button (if space available)
	if (dualPaneState.panes.length < dualPaneState.maxPanes) {
